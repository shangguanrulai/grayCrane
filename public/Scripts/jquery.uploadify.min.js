...
SWFUpload.prototype.getFlashHTML = function() {
    return['<object id="', this.movieName, '" type="application/x-shockwave-flash" data="', this.settings.flash_url, '?v="'+Math.random()+'" width="', this.settings.button_width, '" height="', this.settings.button_height, '" class="swfupload" onclick="return false">', '<param name="wmode" value="', this.settings.button_window_mode, '" />', '<param name="movie" value="', this.settings.flash_url, '" />', '<param name="quality" value="high" />', '<param name="menu" value="false" />', '<param name="allowScriptAccess" value="always" />', '<param name="flashvars" value="' + this.getFlashVars() + '" />', "</object>"].join("");
};
...
(function(c) {
    var b = {init: function(d, e) {
            return this.each(function() {
                ...
                if (h) {
                    ...
                    i.queueData = {files: {}, filesSelected: 0, filesQueued: 0, filesReplaced: 0, filesCancelled: 0, filesErrored: 0, uploadsSuccessful: 0, uploadsErrored: 0, averageSpeed: 0, queueLength: 0, queueSize: 0, uploadSize: 0, queueBytesUploaded: 0, uploadQueue: [], errorMsg: "一些文件不能成功添加，具体原因:"};
                    i.original = m;
                    i.wrapper = l;
                    i.button = f;
                    i.queue = k;
                ...
                if (e[0]) {
                    if (e[0] == "*") {
                        ...
                        for (var m = 0; m < e.length; m++) {
                            i.cancelUpload(e[m]);
                           delete i.queueData.files[e[m]];
                            //delete swfuploadify.queueData.files[e[m]];
    ...
    var a = {onDialogOpen: function() {
            var d = this.settings;
            this.queueData.errorMsg = "以下文件不能成功上传，具体:";
           ...
           if (i.itemTemplate == false) {
                i.itemTemplate = '<li class="pic-item" id="${fileID}_file"><img width="120" height="120" alt="" src="http://icon.fengniao.com/fn_mall/photo.jpg"><i class="${fileID}"></i><span class="cancel"><a href="javascript:$(\'#${instanceID}\').uploadify(\'cancel\', \'${fileID}\')">X</a></span><div id="${fileID}" class="uploadify-queue-item clearfix"><span class="data"></span><span class="fileName">${fileName} (${fileSize})</span><div class="uploadify-progress"><div class="uploadify-progress-bar"><!--Progress Bar--></div></div></div></li>';
            }
            if (c.inArray("onSelect", i.overrideEvents)) {
                itemHTML = i.itemTemplate;
                for (var m in itemData) {
                    itemHTML = itemHTML.replace(new RegExp("\\$\\{" + m + "\\}", "g"), itemData[m]);
                }
                c("#goods_image").append(itemHTML);
            }
                ...
                switch (g) {
                    case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
                        if (e.queueSizeLimit > f) {
                            ...
                            this.queueData.errorMsg += "\n选定的文件数量超过该队列的大小限制 (" + e.queueSizeLimit + ").";
                        }
                        break;
                    ...
                    case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                        ...
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n文件 "' + d.name + '" 不支持此文件类型 (' + e.fileTypeDesc + ").";
                        break;
                    }
            }
            ...
            if (c.inArray("onUploadError", f.overrideEvents) < 0 && e != null ) {
